...
func (c *Client) Authenticate(cmsg []byte) (amsg []byte, err error) {
	...
	if c.User != "" || c.Password != "" || c.Hash != nil {
		...
		// Compute MIC over Negotiate + Challenge + Authenticate (with zeroed MIC field)
		h = hmac.New(md5.New, session.exportedSessionKey)
		h.Write(c.nmsg)
		h.Write(cmsg)
		h.Write(amsg)
		copy(amsg[72:88], h.Sum(nil)) // Write MIC to amsg[72:88]
...
// NegotiateMessage represents an NTLM Negotiate message
...
// NewNegotiateMessage creates an NTLM Negotiate message
func NewNegotiateMessage(domain, workstation string) *NegotiateMessage {
	...
	data, _ := c.Negotiate()
	return &NegotiateMessage{data: data}
}

// CreateAuthenticateMessage creates an NTLM Authenticate message from a challenge
func CreateAuthenticateMessage(challenge []byte, username, password, domain string, lmHash, ntHash, cbt []byte) ([]byte, error) {
	c := &Client{
		User:        username,
		Password:    password,
		Domain:      domain,
