...
// (AS, TGS) through a custom transport. Required by every client constructor
// so that proxy-bypassing leaks are a compile error, not a runtime surprise.
// network is "tcp" or "udp"; address is "host:port" (host may be a hostname).
...
// dialer must be non-nil; pass DirectDialer{} for the legacy net.DialTimeout behavior.
func NewWithPassword(dialer KDCDialer, username, realm, password string, krb5conf *config.Config, settings ...func(*Settings)) *Client {
	if dialer == nil {
		panic("gokrb5 client: KDCDialer is required (pass client.DirectDialer{} for legacy direct net.Dial)")
	}
...
// dialer must be non-nil; pass DirectDialer{} for the legacy net.DialTimeout behavior.
func NewWithKeytab(dialer KDCDialer, username, realm string, kt *keytab.Keytab, krb5conf *config.Config, settings ...func(*Settings)) *Client {
	if dialer == nil {
		panic("gokrb5 client: KDCDialer is required (pass client.DirectDialer{} for legacy direct net.Dial)")
	}
	creds := credentials.New(username, realm)
...
func NewFromCCache(dialer KDCDialer, c *credentials.CCache, krb5conf *config.Config, settings ...func(*Settings)) (*Client, error) {
	if dialer == nil {
		panic("gokrb5 client: KDCDialer is required (pass client.DirectDialer{} for legacy direct net.Dial)")
	}
	cl := &Client{
