...
import (
	...
	. "gopacket/pkg/thirdparty/smb2/internal/erref"
	. "gopacket/pkg/thirdparty/smb2/internal/smb2"

	"gopacket/pkg/thirdparty/smb2/internal/msrpc"
)
...
// SetFileAttributes sets the file attributes directly using a uint32 bitmask.
// This allows setting any combination of FILE_ATTRIBUTE_* flags.
func (fs *Share) SetFileAttributes(name string, attrs uint32) error {
	name = normPath(name)

	if err := validatePath("setattr", name, false); err != nil {
		return err
	}
	...
	f, err := fs.createFile(name, create, true)
	if err != nil {
		return &os.PathError{Op: "setattr", Path: name, Err: err}
	}
...
// SetFileTimes sets file timestamps (creation, access, write, change times).
// Pass nil for any time you don't want to modify.
func (fs *Share) SetFileTimes(name string, creationTime, lastAccessTime, lastWriteTime, changeTime *time.Time) error {
	name = normPath(name)

	if err := validatePath("settimes", name, false); err != nil {
		return err
	}
	...
	f, err := fs.createFile(name, create, true)
	if err != nil {
		return &os.PathError{Op: "settimes", Path: name, Err: err}
	}

	encoder := &FileBasicInformationEncoder{}
	if creationTime != nil {
		encoder.CreationTime = NsecToFiletime(creationTime.UnixNano())
	}
	if lastAccessTime != nil {
		encoder.LastAccessTime = NsecToFiletime(lastAccessTime.UnixNano())
	}
	if lastWriteTime != nil {
		encoder.LastWriteTime = NsecToFiletime(lastWriteTime.UnixNano())
	}
	if changeTime != nil {
		encoder.ChangeTime = NsecToFiletime(changeTime.UnixNano())
	}
...
// RawBytes implements Encoder for raw byte slices
type RawBytes []byte

func (r RawBytes) Size() int {
	return len(r)
}

func (r RawBytes) Encode(b []byte) {
	copy(b, r)
}

// Transact performs an FSCTL_PIPE_TRANSCEIVE operation on the pipe.
// This is an atomic write+read operation used for DCE/RPC over named pipes.
func (f *File) Transact(input []byte, maxOutput int) ([]byte, error) {
	f.m.Lock()
	defer f.m.Unlock()
